<?php 
	...
	if (isset($_SESSION['tapp_par_username'])) {
		print '<script type="text/javascript"> self.location = "dashpanel/" </script>';
	}
...
.bg-black {background: url(../img/user_bg.jpg) repeat center center fixed;
  ...
  background-size: cover;}
    ...
    <body class="bg-black">
		...
		<div class="header"> <a href="../"><?php $kas_framework->displaySchoolLogo('60', 'circle', '0 5px 0 -10px'); ?></a>Parents Check In</div>
			
		<?php if ($kas_framework->app_config_setting('parent_login') == false) {
		$kas_framework->showDangerCallout('<font color="black"><center> Sorry! Parent Login has been closed. See the School Administrator for this.  But Teranig have a question for you... How did you get here? Your IP has been Logged. </font></center>
				<br /><br /><center><a href="'.$kas_framework->url_root('').'" class="btn bg-custom text-white btn-block" style="width:50%">
					...
					require (constant('single_return').'php.files/classes/mailing_list.php');
						...
						<br />Destination: Parent Portal. <br />Location: Login. <br />User IP: '.$kas_framework->getUserIP().'<br />Please Respond.');
			} else { ?>
		 <form action="" method="post" id="loginForm">
                <div class="body bg-gray">
                    <div class="form-group">
					Username or Email
                        <input type="text" required="required" name="username" class="form-control" placeholder="Username or Email"/>
                    </div>
                    <div class="form-group">
					Password
		...
		$('#signin').click(function(e) {
			...
			$.post('../php.files/parents_login_script', loginVals, function(data) {
