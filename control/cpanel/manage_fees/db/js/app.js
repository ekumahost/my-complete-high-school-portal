...
var editableGrid = new EditableGrid("school_fees", {
...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		addCellValidator("price", new CellValidator({ 
			isValid: function(value) { return value == "" || (parseInt(value) >= 0 && parseInt(value) < 999999999); }
		}));
...
$.ajax({// httprequest will post the new data to the server to update db
		...
		async: true // yea, so the admin wont kknow what we are doing go asynchronous
			...
			displayMessage("Value for '" + this.getColumnName(columnIndex) + "' for Component with serial number " + this.getRowId(rowIndex) + " has changed from '" + oldValue + "' to '" + newValue + "'");
		...
		rowSelected = function(oldRowIndex, newRowIndex) {
			if (oldRowIndex < 0) displayMessage("Selected component id: '" + this.getRowId(newRowIndex) + "'");
			else displayMessage("Selected Component id has changed from '" + this.getRowId(oldRowIndex) + "' to '" + this.getRowId(newRowIndex) + "'");
		};
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a onclick=\"if (confirm('Are you sure you want to hide this component ? ')) { editableGrid.remove(" + cell.rowIndex + "); editableGrid.renderCharts(); } \" style=\"cursor:pointer\">" +
							 "<img src=\"" + image("delete.png") + "\" border=\"0\" alt=\"delete\" title=\"Hide Component\"/></a>";
...
EditableGrid.prototype.onloadXML = function(url) 
{
	...
	this.tableLoaded = function() { 
		displayMessage("DATA loaded from database: " + this.getRowCount() + " Component(s)"); 
		this.initializeGrid();
	};
...
{
	this.renderBarChart("barchartcontent", 'Component per Grades' + (this.getRowCount() <= this.maxBars ? '' : ' (first ' + this.maxBars + ' Components out of ' + this.getRowCount() + ')'), 'component', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 1 ? 270 : 0 });
	
	this.renderPieChart("piechartcontent", 'Fees distribution', 'price', 'price');
};
