<?php 			/* deducing the position in class -- the ultimate way of doing things */
			$rawPositionDeducer = "SELECT student, AVG(ca_score1 + ca_score2 + exam_score) as ttl_avg 
						FROM grade_history_primary WHERE level_taken LIKE '".$grade_taken."' AND exam_type='1' 
							AND quarter LIKE '%%' AND year = '".$deduce_year."' group by student order by ttl_avg DESC";
