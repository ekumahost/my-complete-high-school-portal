	...
	<!-- Right side column. Contains the navbar and content of the page -->
		<aside class="right-side">
			...
			<section class="content-header">
				<h1> <i class="fa fa-comments"></i> Discussion <?php $student->display_accessLevel(); ?> </h1>
				<ol class="breadcrumb">
...
<!------- all the posts both image and text ---><span id="allPostHolder">
		...
		<?php 
				$total_shit = $kas_framework->countRestrict1('student_post', 'status', '1');
				//gotten from the previous clicks of the load more......
				$queryLoader = ($_SESSION['loadDiscussionData'] + 1) * 20;				
				$loadRawQuery = "SELECT * FROM student_post WHERE status = '1' ORDER BY id DESC LIMIT 0, ".$queryLoader."";
				$db_loadRawQuery = $dbh->prepare($loadRawQuery);
				$db_loadRawQuery->execute();
				$get_rows_loadRawQuery = $db_loadRawQuery->rowCount();				
				
				if ($get_rows_loadRawQuery == 0) { print '<br /><br />'; $kas_framework->showDangerCallout('Nobody Has Said Anything Here. Be the First'); }
