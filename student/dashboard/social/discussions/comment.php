...
<?php require (constant('tripple_return').'inc.files/sidebar.php') ?>
	<!-- Right side column. Contains the navbar and content of the page -->
		<aside class="right-side" style="min-height:1200px">
			<!-- Content Header (Page header) -->
			<section class="content-header">
				<h1> <i class="fa fa-comments-o"></i> Comment<?php $student->display_accessLevel(); ?> </h1>
				<ol class="breadcrumb">
		...
		<div class="col-md-9">	
			<?php //retrieveing the details of the post,
				$getAllPosts = "SELECT * FROM student_post WHERE id = '".$passingID."' LIMIT 1";
					$db_getAllPosts = $dbh->prepare($getAllPosts);
					$db_getAllPosts->execute();
					$get_rows_db_getAllPosts = $db_getAllPosts->rowCount();
				...
				require (constant('quad_return').'php.files/classes/PHPMailer/PHPMailerAutoload.php');
					require (constant('quad_return').'php.files/classes/mailing_list.php');
						...
						<br />Destination: Student Portal. <br />Location: Discussion Comment. <br />User IP: '.$kas_framework->getUserIP().'<br />
	...
	<ul class="discussion2">
		...
		$pstr_details = "SELECT * FROM studentbio WHERE studentbio_id = '".$show_post->poster_id."'";
		$db_pstr_details = $dbh->prepare($pstr_details);
		$db_pstr_details->execute();
		$paramGetFields = $db_pstr_details->fetch(PDO::FETCH_OBJ);
		...
		$like_all_commas = $kas_framework->getValue('liker_id', 'student_post', 'id', $show_post->id);
...
<div style="border-bottom:1px solid #000; margin:20px 0 10px 8px; border-color:#ccc; width:70%;"> 
 ...
 <?php  
		$querySPost = "SELECT * FROM student_post_reply AS spr, studentbio AS s 
					WHERE spr.post_rel_id = '".$passingID."' AND s.studentbio_id = spr.post_commenter_id ORDER BY spr.id";
					
		$db_querySPost = $dbh->prepare($querySPost);
		$db_querySPost->execute();
		...
		<span id="writeText">
			...
			<form action="" method="post" id="comment_form">
				<input type="text" placeholder="Please Comment on this" id="commentText" />
					<button type="submit" class="btn btn-default btn-flat" >Comment</button>
			</form>	
