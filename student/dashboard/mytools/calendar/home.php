		...
		<?php require (constant('tripple_return').'inc.files/sidebar.php') ?>
            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1><i class="fa fa-calendar-o text-maroon"></i> Calendar <?php $student->display_accessLevel(); ?> </h1>
                    <ol class="breadcrumb">
			...
			<button class="btn btn-success btn-flat" id="confirm_yes" d_id="'.$kas_framework->unsaltifyID($_GET['eventid']).'"><i class="fa fa-thumbs-o-up"></i> Yes</button>');
				...
				<!---the task studffs-->
					...
					<?php			
							$allEventsAll = "SELECT * FROM student_calendar WHERE creator_id = '".$student_id_original."'";
							$db_handle = $dbh->prepare($allEventsAll);
							$db_handle->execute();
							$total_shit = $db_handle->rowCount();
							
							$allEventsAll .= " ORDER BY id DESC LIMIT 0, 6";
							$db_handleX = $dbh->prepare($allEventsAll);
							$db_handleX->execute();
           ...
           $("#datemask2").inputmask("dd/mm/yyyy", {"placeholder": "dd/mm/yyyy"}); 
			$(function() {
                ...
                var d = date.getDate(),
                    ...
                    events: [
					<?php 
					$getAllEvents = "SELECT * FROM student_calendar WHERE creator_id = '".$student_id_original."'";
					$db_handle = $dbh->prepare($getAllEvents);
					$db_handle->execute();
