		...
		<!-- Right side column. Contains the navbar and content of the page -->
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1><i class="fa fa-picture-o text-navy"></i> My Class Gallery <?php $student->display_accessLevel(); ?></h1>
                    <ol class="breadcrumb">
		...
		<?php $student->checkBasicPlanStudent(); //$student->authConfirm($useradmitStatus); ?>
			...
			<div class="row"> 
				...
				<div class="discussion-item">
					...
					<h3 class="discussion-header"><?php print $kas_framework->getValue('grades_desc', 'grades', 'grades_id', $user_student_grade_year_grade_id); ?> Class Pictures &raquo;
					<?php print $student->countallStudentsinMyGallery($user_student_grade_year_grade_id, $current_year_id). ' in Total' ?></h3>
	
					<!--------------------this is just a demancation ----->
					<div class="discussion-body">
						<?php
						/*checkin and makiny sure that the users full name is already completed in the profile*/
						if ($userlastname == '' or $userfirstname == '') {
						print '<br />';
							...
							$allgalleryPicx = "SELECT * FROM student_grade_year AS sgy, studentbio AS sb 
											WHERE sgy.student_grade_year_grade = '".$user_student_grade_year_grade_id."' AND sgy.student_grade_year_year = '".$current_year_id."'
											AND sb.studentbio_pictures != 'empty' AND sb.studentbio_fname != '' AND sb.studentbio_lname != '' AND sb.studentbio_id = sgy.student_grade_year_student";
								...
								for ($i=0; $i<=30; $i++) {
									while ($showPicx = $db_runGalleryPicx->fetch(PDO::FETCH_OBJ)) {
										print '<img src="'.$kas_framework->url_root('pictures/').$showPicx->studentbio_pictures.'" 
										alt="'.$showPicx->studentbio_fname .' '. $showPicx->studentbio_lname.'" 
										title="'.$showPicx->studentbio_lname .' '. $showPicx->studentbio_fname.'"
										href="'.$kas_framework->url_root('pictures/').$showPicx->studentbio_pictures.'" data-fancybox-group="gallery" style="cursor:pointer" class="fancybox fancybox.image margin" />';
									}
								}
