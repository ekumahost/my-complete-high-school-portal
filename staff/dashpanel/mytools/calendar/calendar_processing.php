...
//print print_r($_POST);
	...
	if ($kas_framework->strIsEmpty($new_event) or $kas_framework->strIsEmpty($color) or $kas_framework->strIsEmpty($datemask) or $kas_framework->strIsEmpty($datemask2)) {
		...
		$kas_framework->buttonController('#addEventToCalendar', 'enable');
			...
			$querySQL = "INSERT INTO staff_calendar (creator_id, event_name, start_date, end_date, event_color) 
							VALUES ('".$web_users_relid."', :new_event, '".$datemask."', '".$datemask2."', '".$color."')";
			$db_querySQL = $dbh->prepare($querySQL);
			$db_querySQL->bindParam(':new_event', $new_event);
			$db_querySQL->execute();
