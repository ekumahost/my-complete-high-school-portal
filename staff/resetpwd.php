                ...
                <section class="content">
                  ...
                  <?php $email_input_form = '<div class="error-page">
					...
					<form role="form" action="" method="post">
							<div class="box-body">
								<div class="form-group">
									...
									<input type="password" required="required" class="form-control" name="password1" id="password1" placeholder="Enter New Password">
								</div>
								<div class="form-group">
									<label for="password2">Confirm Password</label>
									<input type="password" required="required" class="form-control" name="password2" id="password2" placeholder="Confirm New Password">
								</div>
					...
					if (isset($_GET['code']) and isset($_GET['email'])) {
						...
						if ($confirmation->confirm_reset_staff($decode_email, $pwd_code) == true) {
						
						$pick_username = "SELECT * FROM web_users AS wu, staff AS s WHERE wu.web_users_relid = s.staff_id AND s.staff_email = '".$decode_email."' AND wu.web_users_password = '".$pwd_code."' LIMIT 1";
						$db_pick_username = $dbh->prepare($pick_username);
						$db_pick_username->execute();
						$paramObj_unuame = $db_pick_username->fetch(PDO::FETCH_OBJ);
						$db_pick_username = null;	
						
						$users_username = $paramObj_unuame->web_users_username;
						...
						$kas_framework->showWarningCallout('Hello <b>'.ucfirst($users_username).':</b> Change your Password. Make sure it is something you can remember');
							(isset($_POST['reset_pwd_final']))? $confirmation->change_password_staff($decode_email, $pwd_code): '';
							echo $password_reset_form;
							
						} else {
							$kas_framework->showDangerCallout('Bad Reset Code. Please Enter your email address again.');
							echo $email_input_form;
						}
					//////////////////////// if not the form, display the email form ///////////////	
					} else {
						(isset($_POST['send_link_butt']))? $confirmation->confirm_email_send_link_staff($_POST['username']): '';
							echo $email_input_form;
					}
