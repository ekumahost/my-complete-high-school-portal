...
if (!isset($_POST['byepass'])) {
	print '<script>  self.location = "home?hacking_attempt_detected" </script>'; exit;
}
			...
			$imagepath = $imagename;
			 ...
			 if (!$image) {
				...
				unlink($file);
					 ...
					 $save = constant('tripple_return')."pictures/student_". $_SESSION['tapp_prostd_username'] .'_'. $imagepath; //This is the new file you saving
					 $file = constant('tripple_return')."pictures/" . $imagepath; //This is the original file
					...
					$Q = "UPDATE studentbio SET studentbio_pictures = 'student_". $_SESSION['tapp_prostd_username'] .'_'. $imagename. "' WHERE studentbio_id = '".$student_id_original."' LIMIT 1";
					$db_Q = $dbh->prepare($Q);
					$db_Q->execute();
					...
					$db_Q = null;	
						if ($get_db_Q_rows == 0) {
							...
							$image_source = '../../../pictures/student_'. $_SESSION['tapp_prostd_username'] .'_'. $imagename. '';
