...
<html>
            ...
            <aside class="right-side" style="min-height:1200px">
                <!-- Content Header (Page header) -->
                 <section class="content-header">
                    <h1><i class="fa fa-tablet text-paleyellow"></i> My Notepad <?php $staff->display_accessLevel(); ?></h1>
                    <ol class="breadcrumb">
	...
	<!-----the content of the notepad--->
		...
		<div class="box-body">
			<ul class="todo-list">
		<?php 
		$thisQ = "SELECT * FROM staff_notepad WHERE author = '".$web_users_relid."' ORDER BY id DESC";
		$db_thisQ = $dbh->prepare($thisQ);
		$db_thisQ->execute();
		$get_thisQ_rows = $db_thisQ->rowCount();
		
			if ($get_thisQ_rows == 0) {
				$kas_framework->showalertwarningwithPaleYellow('No Note Found for you '.$_SESSION['tapp_staff_username'].'. Please try Creating One');
			} else {
				$sn = $get_thisQ_rows;
				$sn = $sn + 1;
					...
					print '<li class="idzez'.$notepad->id.'">
						...
						<a href="#" class="btn btn-primary btn-sm" value="'.$notepad->id.'" style="margin-top:-3px"><i class="fa fa-circle-o"></i> View</a>
