...
<html>
    ...
    <body class="skin-blue">
		...
		<?php require (constant('double_return').'inc.files/sidebar.php') ?>
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        <i class="fa fa-desktop"></i> Library | Staff
                    </h1>
                    <ol class="breadcrumb">
			...
			<!-- Main content -->
				...
				<?php
					...
					if (isset($_GET['create'])) {
						echo '<a href="manageStaff" class="click_hide"><button class="btn bg-ash btn-flat margin">
						<i class="fa fa-chevron-up"></i> Hide the Add Record Panel</button> </a>';
					} else {
						echo '<a href="manageStaff?create" class="click_ult"><button class="btn bg-ash btn-flat margin">
						<i class="fa fa-chevron-down"></i> Activate the Add Record Panel</button> </a>';
					}
				?>
						
				<a href="<?php print constant('single_return') ?>handleLibrary/" class="click_ult"><button class="btn bg-ash btn-flat margin">
						<i class="fa fa-chevron-left"></i> Go to Library Home</button> </a>		
						...
						if (isset($_GET['action'])) {
							...
							/* end of discussions */
								...
								print '<div class="col-md-6">
										<form role="form" action="" method="post" id="addStaffRecordForm">
										<div class="box box-info">
											...
											<div class="box-body">
												<div class="form-group">
													...
													<select name="staffid" required="required" class="form-control"><option></option>';
													$qrun = "SELECT * FROM staff";
													$db_qrun = $dbh->prepare($qrun);
														$db_qrun->execute();
														while ($rw = $db_qrun->fetch(PDO::FETCH_OBJ)) {
															print '<option value="'.$rw->staff_id.'">' .$rw->staff_fname. ' ' .$rw->staff_lname. '</option>';
															}
														$db_qrun = null;
													echo '</select>
												</div>
												...
												<div class="form-group">
													...
													<input type="text" required="required" id="datemask" class="form-control" name="dateout" />
												</div>
												
												<div class="form-group">
													<label>Short Note</label>                                         
													<textarea required="required" class="form-control" name="note" rows="2" cols="10" placeholder="Enter a Short Note" ></textarea>
                            ...
                            <div class="box">
                                ...
                                <div class="box-body table-responsive">
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php  
												$library = "SELECT * FROM media_history WHERE media_history_borrower_type = 'stf'
												...
												$sn = 0;
													...
													$studentfullname = $kas_framework->getValue('staff_fname', 'staff', 'staff_id', $librarybooks->media_history_borrower). '
																	'.$kas_framework->getValue('staff_lname', 'staff', 'staff_id', $librarybooks->media_history_borrower);
														print '<tr>
															<td>'.$sn.'</td>
        ...
        <script type="text/javascript">
			...
			/* add books form */
			$('#addStaffRecordForm').submit(function(e) {
				$('#message_for_addbook').html('<?php $kas_framework->loading_h(); ?>');
				
				var mydata = $('#addStaffRecordForm :input').serializeArray();		
				$.post('borrower_processor?instruction=addrecordZs', mydata, function(data) {
