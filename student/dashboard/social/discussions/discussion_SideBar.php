	<?php 
		$post_count = $kas_framework->countRestrict1('student_post', 'poster_id', $userid);
		$comment_count = $kas_framework->countRestrict1('student_post_reply', 'post_commenter_id', $userid);
		
		$likes_count = "SELECT liker_id FROM student_post";
			$db_handle_like = $dbh->prepare($querySQL);
			$db_handle_like->execute();
			$likes_count_count = 0;
				while ($stream_like = $db_handle_like->fetch(PDO::FETCH_OBJ)) {
					//$likes_count_count = $likes_count_count + substr_count($stream_like->liker_id, $student_id_original. ';');
				}
			$db_handle_like = null;
	//(substr_count($like_all_commas, $student_id_original. ';')
	 ?>
	<div class="col-md-3 no-print fling">
				<div class="box box-primary border_bttn">
					...
					<?php  
					$kas_framework->getMessageforUser('student'); ?>
			<p style="font-size:15px; margin:7px 0 5px 8px; width:80%"> 
				...
				<?php 	 $getImgQuery = "SELECT * FROM studentbio WHERE studentbio_pictures != '' ORDER BY RAND()";
							...
							$get_rows_getImgQuery = $db_getImgQuery->rowCount();
							$paramGetFieldsObj = $db_getImgQuery->fetch(PDO::FETCH_OBJ);
							$db_getImgQuery = null;
				
								if ($get_rows_getImgQuery == 0) {
									$randImgLoc = $kas_framework->url_root('img/user_bg.jpg');
								} else {
									$usrImg = $paramGetFieldsObj->studentbio_pictures;
									$usrSex = $paramGetFieldsObj->studentbio_gender;
