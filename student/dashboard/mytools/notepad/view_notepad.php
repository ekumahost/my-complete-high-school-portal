...
extract($_POST);
	...
	$rawQ = "SELECT * FROM student_notepad WHERE id = '".$value."' AND author = '".$student_id_original."' LIMIT 1";
	$db_rawQ = $dbh->prepare($rawQ);
	$db_rawQ->execute();
	$paramObj_db_rawQ = $db_rawQ->fetch(PDO::FETCH_OBJ);
	$db_rawQ = null;

print '<div class="box box-primary">
		<div class="box-header">
			<i class="fa fa-pencil-square-o"></i>
			<h3 class="box-title">'.$paramObj_db_rawQ->title.'</h3>
				...
				<button class="btn btn-warning btn-sm" id="edit" value="'.$paramObj_db_rawQ->id.'">
				<i class="fa fa-pencil"></i> Edit</button>&nbsp;
				<button class="btn btn-danger btn-sm" id="delete" value="'.$paramObj_db_rawQ->id.'">
				<i class="fa fa-trash-o"></i> Delete</button>&nbsp;
				</span>
		</div>

		<div class="box-body" style="text-align:justify">'.$paramObj_db_rawQ->note.' </div>
</div>';
