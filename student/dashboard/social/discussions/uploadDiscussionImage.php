...
if (!isset($_POST['byepass'])) {
	...
	$target = $target_file. $imagename;
		...
		if (strlen(trim($imagename)) == 0) {
			...
			print '<pre><code>Incorrect File Extension. File should be in JPEG/PNG Format Only. Detected type: '.$kas_framework->fileTypeDetect($type).'. <a href="'.$kas_framework->help_url('?topic=incorrect-file-extension').'" target="_blank">Explanation?</a></code></pre>';
			...
			$imagepath = $imagename;
			 ...
			 $save = $target_file. "Discussion_".$randno.'_'.$_SESSION['tapp_std_username'].'_'.$imagepath; //This is the new file you saving
			 $file = $target_file . $imagepath; //This is the original file
		...
		$insert_link_to_db = "Discussion_".$randno.'_'.$_SESSION['tapp_std_username'].'_'.$imagename;
			 
			$insertImage = "INSERT INTO student_post (poster_id, post_image, post_date) VALUES ('".$student_id_original."', '".$insert_link_to_db."', '".date('d/m/Y')."')";
				$db_insertImage = $dbh->prepare($insertImage);
				$db_insertImage->execute();
				$get_rows_insertImg = $db_insertImage->rowCount();
				$db_insertImage = null;
