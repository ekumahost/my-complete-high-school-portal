...
<head>
	...
	<meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<!---- page description -->
	<meta content="Parent Registration. Check Up and get access to your childs Discipline, Health Report, Result, Attendance, Biography and lots more. Powered by Hypertera" name="description">
...
<body class="bg-black">
	...
	<form role="form" id="registration_form">
		<section class="content" style="max-width:800px; margin:50px auto 0 auto;">
		  <div style="text-align:center; font-size:24px">New Parent Registration</div>
		  
			<div id="signupMessage" align="center"></div>
			<?php 
			if ($kas_framework->app_config_setting('parent_registration') == false) {
			$kas_framework->showDangerCallout('<center> Sorry! Parent Registration has been closed. See the School Administrator for this. But Teranig have a question for you... How did you get here? Your IP has been Logged. </center>
			    ...
			    require (constant('double_return').'php.files/classes/PHPMailer/PHPMailerAutoload.php');
					require (constant('double_return').'php.files/classes/mailing_list.php');
						...
						<br />Destination: parent Portal. <br />Location: Registration. <br />User IP: '.$kas_framework->getUserIP().'<br />Please Respond.');
				} else { ?>
					...
					<div class="box box-primary">
						...
						<div class="box-body">
							 ...
							 <div class="form-group">
								...
								<input type="email" id="email" required="required" name="email" class="form-control" placeholder="Email Account" />
					...
					<div class="box box-success">
						...
						<div class="form-group">
									...
									<label><font color="red">*</font> Mobile (11 Digits)</label>
										<input type="text" name="mobile_no" class="form-control" placeholder="Mobile Number" maxlength="11" required="required" />
									</div>
								<div class="form-group">
									<label><font color="red">*</font> Choose Username (Uneditable) eg. John </label>
										<input type="text" required="required" id="web_parents_username" name="web_parents_username" class="form-control" placeholder="Chose Username" maxlength="20" />
		...
		<script type="text/javascript">
			...
			/*username check */
			$('#web_parents_username').focusout(function(e) {
				web_parents_username = $(this).val();
				byepass = 'niBGYTTcwercxCVV';
				$.post('formValidator?type=web_parents_username', {web_parents_username:web_parents_username, byepass:byepass}, function(data) {
		...
		$('#registration_form').submit(function(e) {
			...
			$('#signupMessage').html('Preparing. <?php $kas_framework->loading_h(); ?>');
				reg_values = $('#registration_form :input').serializeArray();
					$.post('parent_register_script', reg_values, function(data) {
